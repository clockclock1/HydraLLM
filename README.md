...
- 上游健康检查会定时刷新管理界面的在线提供商状态；有模型名时会发送轻量 chat 请求，不只检查 `/models`。
...
默认熔断策略是单个上游连续失败 3 次后禁用 10 分钟；这两个值可在管理界面侧边栏修改并保存到 `data/config.json` 的 `circuitBreaker.failureThreshold` 和 `circuitBreaker.cooldownMinutes`。`429` 会立即进入冷却，避免同一个限流上游被反复命中。代理也会识别 HTTP 200 但响应体内包含 OpenAI 风格 `error` 或 `returned 429` 的伪成功错误。如果某条转移链的所有上游都处于熔断状态，代理会自动解除该链的熔断并重新尝试整条链。
...
- Provider health checks refresh the online-provider status in the management UI on a timer; when a model name is available, HydraLLM sends a lightweight chat request instead of only checking `/models`.
