...
- 管理界面可手动检查上游 `/models` 状态；真实请求过程会被动识别上游故障并触发转移。
...
- The management UI can manually check upstream `/models` status; real proxy requests passively detect upstream failures and trigger failover.
